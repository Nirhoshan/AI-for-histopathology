...
#handling class imbalance when training CNN models on different datasets concluded that, in almost all cases,
# the best strategy was oversampling the minority class(es); increasing the frequency that images from these classes
# are seen by the model during training.
...
def get_dataloader(opt):
    ...
    elif opt.dataset=='mouse':
        train_loader, train_dataset, val_loader, val_dataset, test_loader, test_dataset = get_mouse_dataloader(
            opt
        )

    elif opt.dataset=='nct100k':
        train_loader, train_dataset, val_loader, val_dataset, test_loader, test_dataset = get_nct100k_dataloader(
            opt
        )
...
#Dataframes are a more efficient way of storing data than datasets. They are faster to process and
# they can be queried in an easier way. A dataframe is an object that is structured like a table with rows and columns.
...
def get_nct100k_dataloader(opt):
    ...
    test_df.to_csv(f'{opt.log_path}/test_patches.csv', index=False)
    ...
    if opt.dataset == 'cam':
        ...
        label_enum = {'ADI':0, 'MUC':1, 'BACK':2, 'LYM':3, 'NORM':4, 'DEB':5, 'MUS':6, 'TUM':7, 'STR':8}
        train_dataset = ImagePatchesDataset(opt, train_df, image_dir=base_folder, transform=transform_train, label_enum=label_enum)
        val_dataset = ImagePatchesDataset(opt, val_df, image_dir=base_folder, transform=transform_valid, label_enum=label_enum)
        test_dataset = ImagePatchesDataset(opt, test_df, image_dir=opt.data_input_dir_test, transform=transform_test, label_enum=label_enum)
